...
@inline φ_coords(grid) = @inbounds compute_polar_coords(grid)[2]

@inline ∂p∂φ_over_r(grid, p) = -sin(φ_coords(grid)) * ∂x(p) + cos(φ_coords(grid)) * ∂y(p)
...
function compute_∂p∂φ_over_r(simulation)
   ...
   compute!(∂p∂φ_over_r(simulation.model.grid, 
			(simulation.model.pressures.pNHS 
			 + simulation.model.pressures.pHY′)
	       	       )
	   )
end

@inline ∂p∂r(grid, p) = (cos(φ_coords(grid)) * ∂x(p) 
			 + sin(φ_coords(grid)) * ∂y(p))

function compute_∂p∂r(simulation)
   #Evaluate derivative of kinematic pressure deviation (p) w.r.t. r

   compute!(∂p∂r(simulation.model.grid, (simulation.model.pressures.pNHS + 
					 simulation.model.pressures.pHY′)
		)
	   )
end

@inline ∂φ_∫bdz_over_r(grid, int_bdz) = (-sin(φ_coords(grid)) * ∂x(int_bdz) 
				         + cos(φ_coords(grid)) * ∂y(int_bdz))
...
@inline ∂r_∫bdz(grid, int_bdz) = (cos(φ_coords(grid)) * ∂x(int_bdz) 
				  + sin(φ_coords(grid)) * ∂y(int_bdz))
