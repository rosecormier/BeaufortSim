...
def QG_Vortex_Stability():
    ...
    params   = Parameters()
    geom     = Geometry("cheb", params)
    geom.Lap = BuildLaplacian(params, geom, discretizeVertical = True)
    
    #Perform 2D discretization of grid
    GridInterior(params, geom, discretizeVertical = True)
    ...
    kφs, nmodes = params.kφs, params.nmodes

    #Initialize arrays to store results of eigen-computations

    growthCheb = np.zeros([kφs.shape[0], nmodes])
    propCheb   = np.zeros([kφs.shape[0], nmodes])
    modesCheb  = np.zeros([kφs.shape[0], 
                       (params.halfNr * params.Nz), nmodes],
                      dtype = complex)
    ...
    for kφ_idx in range(0, kφs.shape[0]):
        ...
        eigValCheb, eigVecCheb = spalg.eig(A, B)
        ...
        omegaCheb  = eigValCheb * kφ #Corresponding omegas for this kφ
        print(omegaCheb[0]) 
        #Store results of direct Cheb solve

        growthCheb[kφ_idx, :]   = -omegaCheb[0:nmodes].imag
        propCheb[kφ_idx, :]     = omegaCheb[0:nmodes].real
        modesCheb[kφ_idx, :, :] = eigVecCheb[:, 0:nmodes]
        """
            t0FD = timeit.timeit()
    ...
    growthDimCheb = growthCheb * params.f0
    propDimCheb   = propCheb * params.f0
    ...
    if nmodes < 3:
        ...
        #Visualize growth rates and propagation speeds of most unstable modes
        
        fig, axes = plt.subplots(nmodes, 2, figsize = (10, 7), sharex = "col")
            
        for ii in range(0, nmodes):
                
            ax_growth = axes[ii, 0]
            ax_growth.plot(kφs, 4 * np.ravel(growthDimCheb[:, ii]), 
                               ".-", color = "mediumpurple", 
                               label = "Cheb solver")
            #ax_growth.plot(kzs, 4 * np.ravel(growthDimFD[:, ii, jj]), ".-", color = "orange", label = "FD solver")
            ax_growth.set(title = 
                              f"Growth rate; mode {ii}",
                              ylabel = "Growth rate ($s^{-1}$)")
                
            ax_prop = axes[ii, 1]
            ax_prop.plot(kφs, 4 * np.ravel(propDimCheb[:, ii]), 
                             ".-", color = "mediumpurple", 
                             label = "Cheb solver")
            #ax_prop.plot(kzs, 4 * np.ravel(propDimFD[:, ii, jj]), ".-", color = "orange", label = "FD solver")
            ax_prop.set(title = 
                        f"Propagation speed; mode {ii}",
        ...
        axes[0, 0].legend()
        plt.show()
        fig.savefig(f"omega_vs_k_nondimensionalBCgyre.png")
        plt.close(fig)
