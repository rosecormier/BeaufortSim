...
def QG_Vortex_Stability():
    ...
    for kφ_idx in range(0, kφs.shape[0]):
        ...
        B_Ch = (geomCh.Lap - np.diag(kφ2 * geomCh.r2Recip)
                + ((1 / paramsCh.Bu) * (geomCh.Dz 
                    * (geomCh.N2Recip * geomCh.Dz)))
               ) / paramsCh.Ro
        A_Ch = np.matmul(geomCh.Ψ_op, (B_Ch * paramsCh.Ro)) - geomCh.Q_op
    ...
    growthDimCh = growthCh * paramsCh.f0
    propDimCh   = propCh * paramsCh.f0
    ...
    if nmodes == 2:
        ...
        fig, axes = plt.subplots(nmodes, 2, figsize = (9, 7), sharex = "col")
        ...
        for ii in range(0, nmodes):
            ...
            ax_prop = axes[ii, 1]
            ax_prop.scatter(kφs, np.ravel(propDimCh[:, ii]), 
                         color = "mediumpurple", label = "Cheb solver")
            ax_prop.set(title = f"Propagation speed; mode {ii}",
        ...
        for jj in range(0, nmodes):
            ...
            for kφ_idx in range(nkφ):
                ...
                eigVecCh_C     = np.reshape(modesCh[kφ_idx, :, jj],
                ...
                figEigVec.suptitle(f"Components of normalized mode-{jj} eigenvectors for $k =$ {kφ}")
                #plt.show()
                figEigVec.savefig(f"eigvec_2D_k{kφ}_mode{jj}_nondimensionalBCgyre.png")
                plt.close(figEigVec)
