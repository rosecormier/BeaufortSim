"""
Plot scalar and vector ECCO fields, monthly and annually.
...
for k in range(kmin, kmax + 1): #Iterate over all specified depths
    ...
    for i in range(years): #Iterate over all specified years
        ...
        #Plot all months
        ArcCir_contourf_quiver_grid(ds_grid, k, scalars, vecEs, vecNs, resolution, vir_nanmasked, yearstr, lon_centers, lat_centers, scalar_bounds=scalar_bounds, outfile=join(outdir, 'yearly', '{}_k{}_all{}.png'.format(variables_str, str(k), yearstr)))

        #Plot annual average
        scalar_mean, vecE_mean, vecN_mean = ArcCir_contourf_quiver(ds_grid, k, scalars, vecEs, vecNs, resolution, vir_nanmasked, yearstr+" average", lon_centers, lat_centers, scalar_bounds=scalar_bounds, outfile=join(outdir, 'yearly', '{}_k{}_avg{}.pdf'.format(variables_str, str(k), yearstr)))
        ...
        ArcCir_contourf_quiver_grid(ds_grid, k, scalar_residuals, vecE_residuals, vecN_residuals, resolution, 'seismic', yearstr, lon_centers, lat_centers, scalar_bounds=[-0.5, 0.5], outfile=join(outdir, 'yearly', '{}_k{}_resids_all{}.png'.format(variables_str, str(k), yearstr)), resid=True)