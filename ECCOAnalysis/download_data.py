"""
Main function:
    -For a specified ECCO field (scalar or vector) and date range, iterates over
    time and downloads the field data;
    -If "seasonal" time-average type is specified, saves seasonal averages of 
    primary data.
...
def compute_seasonal_average(monthly_fields, datdir_primary, field_name, 
    ...
    if monthly_fields[1] is None:
        ...
        seasonal_avg_0 = compute_temporal_mean(monthly_fields[0]).squeeze()
        seasonal_avg_1 = compute_temporal_mean(monthly_fields[1]).squeeze()
...
def main(**kwargs):
    ...
    if kwargs:
        ...
        initial_month = kwargs.get('initial_month')
        initial_year = kwargs.get('initial_year')
    ...
    if time_ave_type == "monthly":
        ...
        while year < int(final_year):
            ...
            while month <= 12:
                ...
                ecco_podaac_download(ShortName=field_shortname, 
        ...
        if year == int(final_year):
            while month <= int(final_month):
                ...
                StartDate = final_year+"-"+monthstr+"-02"
                EndDate = final_year+"-"+monthstr+"-"+endmonth
                ecco_podaac_download(ShortName=field_shortname, 
                                     StartDate=StartDate, EndDate=EndDate, 
                                     download_root_dir=datdir_primary, 
                                     n_workers=6, force_redownload=False)
        ...
        if season_start < season_end:
            ...
            while year <= int(final_year):
                ...
                while month <= season_end:
                    ...
                    StartDate = date_string + "-02"
                    EndDate = date_string + "-" + endmonth
                    ecco_podaac_download(ShortName=field_shortname, 
                                         StartDate=StartDate, EndDate=EndDate, 
                                         download_root_dir=datdir_primary, 
                                         n_workers=6, force_redownload=False)
                    ds_month = load_data_files.main(field_name=field_name, 
                                                    date_string=date_string, 
                                                    datdir_primary=
                                                    datdir_primary, 
                                                    time_ave_type='monthly') 
                        
                    try: #For vector fields
                        attributes = get_vector_comps(field_name) 
                        if monthly_fields_0 is None:
                            monthly_fields_0 = ds_month[attributes[0]]
                            monthly_fields_1 = ds_month[attributes[1]]
                        elif monthly_fields_0 is not None:
                            monthly_fields_0 = xr.concat([monthly_fields_0, 
                                                          ds_month[attributes
                                                                   [0]]], 
                                                         'time')
                            monthly_fields_1 = xr.concat([monthly_fields_1, 
                                                          ds_month[attributes
                                                                   [1]]], 
                                                         'time')
                    except:
            ...
            while year <= int(final_year):
                ...
                if ((year != int(final_year) and season_start <= month) 
                    or (year != int(initial_year) and month <= season_end)):
